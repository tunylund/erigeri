...
import { collisions, interpolationMultiplier } from './assets.mjs'
import { reduce, sample, intersects, distance } from './utils.mjs'
import Q from './Q.mjs'

function spotAttack(target) {
  if(target.p.attacking && target.p.animationFrame > 4 * interpolationMultiplier) {
...
class AutoPlayer extends GeriMon {
  ...
  cancelAttack () {
    return
    if(this.p.attacking && this.p.animationFrame < 4 * interpolationMultiplier) {
      this.stand()
    }
  }
  ...
  cancelUnsoku () {
    if(this.p.walking) {
      if(this.p.animationFrame < 3 * interpolationMultiplier || this.p.animationFrame > 6 * interpolationMultiplier) {
        this.stand()
      }
    }
  }
