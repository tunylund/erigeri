...
function buildAnimations() {
  Q.animations('gerimon', {
    stand: { frames: [0, 1] },
    senten: { frames: range(Q.sheet('senten-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    koten: { frames: range(Q.sheet('koten-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    sentainotsuki: { frames: range(Q.sheet('sentainotsuki-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    fujogeri: { frames: range(Q.sheet('fujogeri-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    suiheigeri: { frames: range(Q.sheet('suiheigeri-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    manjigeri: { frames: range(Q.sheet('manjigeri-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    hangetsuate: { frames: range(Q.sheet('hangetsuate-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    sensogeri: { frames: range(Q.sheet('sensogeri-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    tsuisoku: { frames: range(Q.sheet('tsuisoku-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    kosoku: { frames: range(Q.sheet('kosoku-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    ushiro: { frames: range(Q.sheet('ushiro-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    ninoashi: { frames: range(Q.sheet('ninoashi-a').frames), rate: 1/24, loop: false, trigger: 'stand' },
    taisoku: { frames: range(Q.sheet('tsuisoku-a').frames).reverse(), rate: 1/24, loop: false, trigger: 'stand' },
    torsohit: { frames: range(8).concat(range(6).reverse()), rate: 1/24, loop: false, trigger: 'stand' },
    headoffhit: { frames: range(Q.sheet('headoff-hit-a').frames), rate: 1/24, loop: false }
  })
}
...
function loadAssets (cb) {
  ...
  async function buildAssets() {
    ...
    playerAssets.map(async name => {
      const canvas = interpolate(Q.asset(`assets/${name}.png`), PLAYER_TILEW)
    ...
    playerAssets
      ...
      .map(name => {
        ...
        const canvas = duplicateFrames(Q.asset(assetName), PLAYER_TILEW)
...
export { collisions, interpolate, loadAssets as default }
