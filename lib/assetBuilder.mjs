...
async function buildAssets() {
  ...
  Q.assets[`assets/tiles.png`] = getAsset('tiles')
  Q.assets[`assets/judge.png`] = getAsset('judge')
  Q.assets[`assets/bg-1.png`] = getAsset('bg-1')
  ...
  await Promise.all(playerAssets
    ...
    .map(async name => {
      const assetName = `${name}-collisions`
      const canvas = reduce(interpolationMultiplier / 2, img => duplicateFrames(img, PLAYER_TILEW), getAsset(assetName))
...
export { collisions, interpolationMultiplier, buildAssets as default }
