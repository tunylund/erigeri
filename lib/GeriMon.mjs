...
class GeriMon extends Q.MovingSprite {
  ...
  hitStep () {
    ...
    let hit = this.hitTest(collisions[this.p.animation].hit[this.p.animationFrame])
    if(hit) {
      playSound('assets/hit-' + sample([1,2,3,4]) + '.mp3')
      let value = this.p.target.hit(this.p.dir * this.hitForce[this.p.animation], hit)
      ...
      this.p.missed = true
      playSound('assets/miss-' + sample([1,1,1,1,1,1,2]) + '.mp3')
    }
  }
