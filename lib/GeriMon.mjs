...
Q.animations('gerimon', {
  stand: { frames: [0, 1] },
  senten: { frames: range(19 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  koten: { frames: range(19 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  sentainotsuki: { frames: range(22 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  fujogeri: { frames: range(15 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  suiheigeri: { frames: range(15 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  manjigeri: { frames: range(15 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  hangetsuate: { frames: range(21 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  sensogeri: { frames: range(20 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  tsuisoku: { frames: range(11 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  kosoku: { frames: range(18 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  ushiro: { frames: range(7 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  ninoashi: { frames: range(6 * 2), rate: 1/24, loop: false, trigger: 'stand' },
  taisoku: { frames: range(11 * 2).reverse(), rate: 1/24, loop: false, trigger: 'stand' },
  torsohit: { frames: [0,1,2,3,4,5,6,7,5,4,3,2,1,0], rate: 1/24, loop: false, trigger: 'stand' },
  headoffhit: { frames: range(13 * 2), rate: 1/24, loop: false }
})
...
const comboFromSensogeri = comboFrom('sensogeri', 13 * 2, 16 * 2)
const comboFromSentainoTsuki = comboFrom('sentainotsuki', 10 * 2, 16 * 2)
const comboFromFujogeri = comboFrom('fujogeri', 10 * 2, 16 * 2)
const comboFromManjigeri = comboFrom('manjigeri', 10 * 2, 13 * 2)
const comboFromSenten = comboFrom('senten', 5 * 2, 13 * 2)
const comboFromKoten = comboFrom('koten', 5 * 2, 13 * 2, function () {
...
function attack(fn) {
  return function(target) {
    ...
    if(this.p.walking && this.p.animationFrame > 4 * 2 || this.p.animation === 'ushiro') return false
    this.sentenEnd()
    this.kotenEnd()
    ...
    this.p.attackEvent = { s: event(this.p), t: event(target.p) }
...
class GeriMon extends Q.MovingSprite {
  ...
  fujogeri (target) {
    return comboFromSentainoTsuki(comboFromSenten(jump(attack(() => {
      ...
      this.play('fujogeri', 1)
      this.on('step', this, 'fujogeriStep')
    })), 4 * 2), 5 * 2).call(this, target)
  }

  fujogeriForwardStep () {
    if(this.p.animationFrame > 4 * 2 && this.p.animationFrame < 7 * 2) {
  ...
  sentenStep () {
    if (this.p.animationFrame > 3 * 2) {
      this.p.vx = this.p.dir * this.speed * 2.5
    }
    if (this.p.animationFrame > 11 * 2) {
  ...
  kotenStep () {
    if (this.p.animationFrame > 2 * 2) {
      this.p.vx = -this.p.dir * this.speed * 2.5
    }
    if (this.p.animationFrame > 12 * 2) {
      this.p.vx = this.p.dir
    }
  }
    ...
    const missed = targets
      .map(target => {
        ...
        if (hit) {
          ...
          AttackLog.push(this.p.attackEvent, value, hit)
        }
        return hit
      })
